                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($recentOrders as $order)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-gray-600">{{ $order->user->name ?? 'N/A' }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-gray-900 font-medium">${{ number_format($order->total, 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($lowStockProducts as $product)
                            <tr class="hover:bg-red-50">
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <a href="{{ route('admin.products.edit', $product) }}" class="text-gray-900 hover:text-indigo-600 font-medium">
                                        {{ Str::limit($product->name, 25) }}
                                    </a>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-gray-500">{{ $product->sku ?? '-' }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-bold {{ $product->stock <= 0 ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800' }}">
                                        {{ $product->stock }}
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="px-4 py-6 text-center text-gray-500">All products are well stocked.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
